...
export const postProd = (newProd) => (dispatch) => {
  ...
  axios
    .post(
      "https://naykaabackenddd.onrender.com/api/products/add",
      newProd, 
      {
        headers: {
...
export const getProd=(paramsobj)=>(dispatch)=>{
    const token=localStorage.getItem("token");
    dispatch({type:PROD_POST_REQ})
    let prod=axios.get("https://naykaabackenddd.onrender.com/api/products",{
        headers: {
          authorization: `Bearer ${token}`,
        },
        params: paramsobj,
...
export const editProd=(id,newProd)=>(dispatch)=>{
    const token=localStorage.getItem("token");
    dispatch({type:PROD_POST_REQ})
    let prod=axios.patch(`https://naykaabackenddd.onrender.com/api/products/update/${id}`,newProd,{
        headers: {
          authorization: `Bearer ${token}`,
        }
      }).then((res)=>{
...
export const deleteProd=(id)=>(dispatch)=>{
    const token=localStorage.getItem("token");
    dispatch({type:PROD_POST_REQ})
    let prod=axios.delete(`https://naykaabackenddd.onrender.com/api/products/delete/${id}`,{
        headers: {
          authorization: `Bearer ${token}`
        }
      }).then((res)=>{
